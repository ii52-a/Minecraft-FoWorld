...
public class LuminexFlowerEntity extends BlockEntity {
    ...
    public int getGlow() { return this.glow; }
    public int getMaxGlow() { return MAX_GLOW; }
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, LuminexFlowerEntity be) {
        ...
        // 1. 【自然流失】：如果不干活，能量也会慢慢消散（模拟能量的不稳定）
        // 每 100 刻（5秒）检查一次。!level.isClientSide 表示只在服务器端算账。
    ...
    private static boolean handle_if_transform(Level level,BlockPos pos){
        return level.getBlockState(pos).is(Blocks.STONE) || level.getBlockState(pos).is(BlockRegistry.FLUORITE_ORE_0.get()) ||level.getBlockState(pos).is(BlockRegistry.FLUORITE_ORE_1.get());
    }
